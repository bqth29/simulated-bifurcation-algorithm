...
def test_use_scenario():
    ...
    # First update
    assert torch.equal(window.previously_bifurcated, torch.Tensor([False, False]))
    window.update(SCENARIO[0])
    assert window.must_continue()
    assert not window.has_bifurcated_spins()
    assert torch.equal(window.get_bifurcated_spins(), torch.Tensor([]))
    assert torch.equal(window.current_spins, SCENARIO[0])
    assert torch.equal(window.final_spins, torch.zeros((3, 2)))
    assert torch.equal(window.stability, torch.Tensor([0, 0]))
    assert torch.equal(window.newly_bifurcated, torch.Tensor([False, False]))
    assert torch.equal(window.bifurcated, torch.Tensor([False, False]))
    assert torch.equal(window.equal, torch.Tensor([False, False]))
    ...
    assert torch.equal(window.newly_bifurcated, torch.Tensor([False, False]))
    assert torch.equal(window.bifurcated, torch.Tensor([False, False]))
    assert torch.equal(window.equal, torch.Tensor([True, False]))
    ...
    assert torch.equal(window.newly_bifurcated, torch.Tensor([True, False]))
    assert torch.equal(window.bifurcated, torch.Tensor([True, False]))
    assert torch.equal(window.equal, torch.Tensor([True, True]))

    # Fourth update
    assert torch.equal(window.previously_bifurcated, torch.Tensor([True, False]))
    ...
    assert torch.equal(window.newly_bifurcated, torch.Tensor([False, True]))
    assert torch.equal(window.bifurcated, torch.Tensor([True, True]))
    assert torch.equal(window.equal, torch.Tensor([True, True]))
