...
def test_use_scenario():
    ...
    # First update
    assert torch.equal(window.previously_bifurcated, torch.Tensor([False, False]).to(dtype=torch.bool))
    ...
    assert torch.equal(window.newly_bifurcated, torch.Tensor([False, False]).to(dtype=torch.bool))
    assert torch.equal(window.bifurcated, torch.Tensor([False, False]).to(dtype=torch.bool))
    assert torch.equal(window.equal, torch.Tensor([False, False]).to(dtype=torch.bool))

    # Second update
    assert torch.equal(window.previously_bifurcated, torch.Tensor([False, False]).to(dtype=torch.float))
    ...
    assert torch.equal(window.newly_bifurcated, torch.Tensor([True, False]).to(dtype=torch.bool))
    assert torch.equal(window.bifurcated, torch.Tensor([True, False]).to(dtype=torch.bool))
    assert torch.equal(window.equal, torch.Tensor([True, True]).to(dtype=torch.bool))

    # Fourth update
    assert torch.equal(window.previously_bifurcated, torch.Tensor([True, False]).to(dtype=torch.float))
    ...
    assert torch.equal(window.newly_bifurcated, torch.Tensor([False, True]).to(dtype=torch.bool))
    assert torch.equal(window.bifurcated, torch.Tensor([True, True]).to(dtype=torch.bool))
    assert torch.equal(window.equal, torch.Tensor([True, True]).to(dtype=torch.bool))
